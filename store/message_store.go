...
type MessageStore interface {
	...
	Shutdown()                                                    // 关闭存储服务
	Destroy()                                                     //
	PutMessage(msg *MessageExtBrokerInner) *core.PutMessageResult //
	GetMessage(group string, topic string, queueId int32, offset int64, maxMsgNums int32, subscriptionData *heartbeat.SubscriptionData) *core.GetMessageResult
	...
	SelectOneMessageByOffset(commitLogOffset int64) *core.SelectMapedBufferResult                             // 通过物理队列Offset，查询消息。 如果发生错误，则返回null
	SelectOneMessageByOffsetAndSize(commitLogOffset int64, msgSize int32) *core.SelectMapedBufferResult       // 通过物理队列Offset、size，查询消息。 如果发生错误，则返回null
	...
	GetCommitLogData(offset int64) *core.SelectMapedBufferResult                                              // 数据复制使用：获取CommitLog数据
	AppendToCommitLog(startOffset int64, data []byte) bool                                                    // 数据复制使用：向CommitLog追加数据，并分发至各个Consume Queue
	ExcuteDeleteFilesManualy()                                                                                //
	QueryMessage(topic string, key string, maxNum int32, begin int64, end int64) *core.QueryMessageResult     //
