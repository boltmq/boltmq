...
app=registry
conf=cfg.json
examplefile=cfg.example.json
pidfile=logs/registry.pid
logfile=logs/registry.log
...
function pack() {
    ...
    version=`./$app -v`
    cp g/$examplefile $examplefile
    packName=$app-$version.tar.gz
    rm -f $packName
    tar -zcvf $packName control.sh $examplefile $app gitversion
    rm -f $examplefile
}
