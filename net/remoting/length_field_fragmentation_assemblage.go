...
type LengthFieldFragmentationAssemblage struct {
	...
	fragmentationTable   map[core.SocketAddr]*bytes.Buffer // 按连接地址对包进行处理，每个连接有独立goroutine处理。
}
...
func NewLengthFieldFragmentationAssemblage(maxFrameLength, lengthFieldOffset, lengthFieldLength, initialBytesToStrip int) *LengthFieldFragmentationAssemblage {
	return &LengthFieldFragmentationAssemblage{
		...
		fragmentationTable:  make(map[core.SocketAddr]*bytes.Buffer),
	}
}
...
func (lfpfa *LengthFieldFragmentationAssemblage) pack(addr core.SocketAddr, buf *bytes.Buffer) (bufs []*bytes.Buffer, e error) {
	var (
		start        int
		end          int
		length       int
...
func (lfpfa *LengthFieldFragmentationAssemblage) adjustBuffer(addr core.SocketAddr, buf *bytes.Buffer, packetLength int) (*bytes.Buffer, error) {
	// buffer中报文长度
